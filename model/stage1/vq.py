...
class EuclideanCodebook(nn.Module):
    ...
    def forward(self, x, svq_temp: Union[float, None] = None):
        ...
        embed = embed.t()  # [dim, codebook_size]
        ...
        # 1. ?? Codebook ???? [1, codebook_size]
        embed_sq = embed.pow(2).sum(0, keepdim=True)

        # 2. ????? [batch_size * seq_len, codebook_size]
...
class VectorQuantize(nn.Module):
    ...
    def get_codebook_entry(self, indices, *kwargs):
        """
        Retrieve codebook vectors based on indices and apply projection.
        Args:
            indices: [Batch, ..., Heads] if heads > 1 else [Batch, ...]
        Returns:
            z_q: [Batch, ..., Dim]
        """
        # 1. Lookup vectors from codebook
        # The internal codebook.embed shape is [Codebook_Size, Codebook_Dim]
        # F.embedding will return [..., Codebook_Dim]
        z_q = F.embedding(indices, self._codebook.embed)

        # 2. Merge multi-head dimensions
        if self.heads > 1:
            # If multi-headed, indices should have last dim == heads
            # z_q shape: [..., heads, codebook_dim]
            # rearrange to [..., heads * codebook_dim]
            z_q = rearrange(z_q, '... h d -> ... (h d)')

        # 3. Output projection
        # Map back to model dimension D
